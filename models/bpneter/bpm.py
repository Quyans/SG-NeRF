...
class Linking(nn.Module):
    ...
    def forward(self, feat_2d_all, feat_3d, links, init_3d_data=None):
        ...
        # current_links[[0],:] = links[[1],:]
        # import copy
        #
        # inc = copy.deepcopy(coords_map_in)
        # out = copy.deepcopy(coords_map_out)
        # clinks = copy.deepcopy(current_links)
        # links = copy.deepcopy(links)
        
        # clinks[out,:] = links[inc,:]

        current_links[coords_map_out, :] = links[coords_map_in, :] #原本的
        # current_links[inc, :] = links[out, :]
